...
def calculate_metrics(basket, recommendations, rules):
    ...
    if relevant_rules:
        metrics['avg_confidence'] = np.mean([r['confidence'] for r in relevant_rules])*130
...
# Interface principal
def main():
    st.markdown('<div class="header">🛒 Supermercado Siqueira </div>', unsafe_allow_html=True)
    st.markdown("""
    <div style='text-align: center; margin-bottom: 2rem; font-size: 1.1rem; color: #555;'>
    Adicione produtos à sua cesta e receba recomendações de produtos
    </div>
    """, unsafe_allow_html=True)
